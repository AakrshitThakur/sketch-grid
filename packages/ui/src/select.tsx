...
function Select<T extends string | number>(props: SelectProps<T>) {
  ...
  // get selected-option
  const selected_option = props.options.find((option) => option.value === props.value);

  return (
    <div ref={container_ref} id="select" className={`flex flex-col gap-1 ${props.class_name}`}>
      ...
      <div className="relative">
        ...
        >
          <span>{selected_option ? selected_option.label : props.placeholder}</span>
          <svg
            ...
            {props.options.map((option) => (
              <div
                ...
                className={`cursor-pointer px-3 py-2 text-sm hover:bg-blue-100 ${
                  props.value === option.value ? "bg-blue-50 text-blue-700 font-medium" : ""
                }`}
              >
                {option.label}
              </div>
